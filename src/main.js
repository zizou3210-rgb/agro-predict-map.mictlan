...
const mapScreenshotButton = document.getElementById("map-screenshot");
const mapZoomInButton = document.getElementById("map-zoom-in");
const mapZoomOutButton = document.getElementById("map-zoom-out");
const mapToggleExpandButton = document.getElementById("map-toggle-expand");
...
const selectionChoiceConfigs = new Map([
  [selectionTargetVariableSelect, { searchEnabled: true, searchPlaceholderValue: "Search target variable", placeholderValue: "Select target variable" }],
  [selectionLongitudeColumnSelect, { searchEnabled: true, searchPlaceholderValue: "Search longitude column", placeholderValue: "Select longitude column" }],
  [selectionLatitudeColumnSelect, { searchEnabled: true, searchPlaceholderValue: "Search latitude column", placeholderValue: "Select latitude column" }],
  [selectionPlantingDateColumnSelect, { searchEnabled: true, searchPlaceholderValue: "Search Date of Plating", placeholderValue: "Select Date of Plating" }],
  [selectionHarvestingDateColumnSelect, { searchEnabled: true, searchPlaceholderValue: "Search Date of Harvesting", placeholderValue: "Select Date of Harvesting" }],
  [selectionSoilTextureColumnSelect, { searchEnabled: true, searchPlaceholderValue: "Search Soil type/texture", placeholderValue: "Select Soil type/texture" }],
  [selectionSoilDepthColumnSelect, { searchEnabled: true, searchPlaceholderValue: "Search Soil Depth (cm)", placeholderValue: "Select Soil Depth (cm)" }],
]);
...
const trainingResultDownloadPanel = document.getElementById("training-result-download-panel");
const trainingResultDownloadMessage = document.getElementById("training-result-download-message");
const trainingResultDownloadLink = document.getElementById("download-training-results");
...
const predictionDownloadModal = document.getElementById("prediction-download-modal");
const predictionDownloadModalBackdrop = document.getElementById("prediction-download-modal-backdrop");
const predictionDownloadModalClose = document.getElementById("prediction-download-modal-close");
const predictionDownloadModalCard = document.getElementById("prediction-download-modal-card");
const predictionDownloadModalDescription = document.getElementById("prediction-download-modal-description");
const predictionDownloadModalMeta = document.getElementById("prediction-download-modal-meta");
const predictionDownloadNameInput = document.getElementById("prediction-download-name-input");
const predictionDownloadExtension = document.getElementById("prediction-download-extension");
const predictionDownloadModalCancel = document.getElementById("prediction-download-modal-cancel");
const predictionDownloadModalSave = document.getElementById("prediction-download-modal-save");
...
selectionChoiceConfigs.set(
  predictionIdColumnSelect,
  { searchEnabled: true, searchPlaceholderValue: "Search id field", placeholderValue: "Select id field", renderSelectedChoices: "always" },
);
selectionChoiceConfigs.set(
  germplasmSelectionIdSelect,
  { searchEnabled: true, searchPlaceholderValue: "Search column id", placeholderValue: "Select column id", renderSelectedChoices: "never" },
);
...
const colorHelpScale = document.getElementById("color-help-scale");
...
let mapFrameExpanded = false;
let mapContext = null;
let loadedFeatures = [];
let predictedYieldStats = null;
let trainingErrorQuartiles = null;
let activePredictionProfileHighlight = "";
let checkedPredictionProfileLabels = new Set();
let predictionProfileSelectionTouched = false;
...
let cePipelineDownloadEnabled = true;
...
let availablePredictionSelectionGroupsByLabel = {};
...
let manualGridOverlayComputationCache = null;
let manualGridServerOverlayCache = new Map();
...
let pendingPredictionDownloadRequest = null;
...
const populateFeatureheroSettingsForm = (settings = featureheroSettings) => {
  ...
  featureheroSettings = {
    number_generation: Number(settings?.number_generation ?? 200),
    number_population: Number(settings?.number_population ?? 120),
...
const syncPersistedPipelineStateWithRuntime = async () => {
...
const extractFilenameFromDownloadUrl = (downloadUrl) => {
...
const splitFilenameParts = (filename) => {
  ...
  if (lastDotIndex <= 0 || lastDotIndex === normalizedName.length - 1) {
    return { stem: normalizedName, extension: "" };
  }
  return {
...
const resolvePredictionOutputDownloadUrl = (payload) => {
  const directUrl = String(payload?.prediction_output_download_url ?? "").trim();
  if (directUrl) {
    return directUrl;
  }
  ...
  return `/api/process-xlsx/artifact/${encodeURIComponent(jobId)}/prediction_output`;
};

const getPredictionJobId = () => String(datasetStore.prediction?.summary?.job_id ?? "").trim();

const buildManualGridOverlaySelectionKey = (labels) =>
  Array.from(labels ?? [])
    .map((label) => normalizePredictionCategoryLabel(label))
    .filter(Boolean)
    .sort()
    .join("|");

const fetchManualGridProfileOverlayFromBackend = async (labels) => {
  const jobId = getPredictionJobId();
  if (!jobId) {
    return null;
  }
  ...
  normalizedLabels.forEach((label) => query.append("labels", label));
  const response = await fetch(
    `/api/process-xlsx/artifact/${encodeURIComponent(jobId)}/profile_overlay?${query.toString()}`,
  );
...
const promptForDownloadFilename = (suggestedFilename = "prediction_output.xlsx") => {
  const { stem, extension } = splitFilenameParts(suggestedFilename);
  const suggestedStem = stem || "prediction_output";
  const suffix = extension || ".xlsx";
  const enteredName = window.prompt(
    "Enter the file name to download the High-Potential Sites prediction results.",
    suggestedStem,
  );
  if (enteredName === null) {
    return null;
  }
  const normalizedEnteredName = String(enteredName).trim();
  if (!normalizedEnteredName) {
    window.alert("The file name cannot be empty.");
    return null;
  }
  return normalizedEnteredName.includes(".")
    ? normalizedEnteredName
    : `${normalizedEnteredName}${suffix}`;
};

const requestPredictionDownloadFilename = async (suggestedFilename = "prediction_output.xlsx") => {
  const normalizedSuggestedFilename =
    String(suggestedFilename ?? "").trim() || "prediction_output.xlsx";
  if (!predictionDownloadModal || !predictionDownloadNameInput) {
    return promptForDownloadFilename(normalizedSuggestedFilename);
  }
  const { stem, extension } = splitFilenameParts(normalizedSuggestedFilename);
  const dialogResult = await openPredictionDownloadModal({
    suggestedName: stem || "prediction_output",
    extension: extension || ".xlsx",
  });
  if (dialogResult?.action !== "download") {
    return null;
  }
  const normalizedEnteredName = String(dialogResult.value ?? "").trim();
  if (!normalizedEnteredName) {
    window.alert("The file name cannot be empty.");
    return null;
  }
  return normalizedEnteredName.includes(".")
    ? normalizedEnteredName
    : `${normalizedEnteredName}${extension || ".xlsx"}`;
};
...
const triggerBrowserDownload = async (
  downloadUrl,
  {
    ...
    suggestedFilename = "prediction_output.xlsx",
    useNativePrompt = false,
    bypassDownloadGate = false,
  } = {},
) => {
  if (!bypassDownloadGate && !cePipelineDownloadEnabled) {
    return;
  }
  const normalizedUrl = String(downloadUrl ?? "").trim();
  if (!normalizedUrl) {
  ...
  try {
    const normalizedSuggestedFilename =
      ...
      || extractFilenameFromDownloadUrl(normalizedUrl)
      || "prediction_output.xlsx";
    let finalFilename = suggestedFilename;
    if (promptForFilename) {
      if (useNativePrompt) {
        finalFilename = promptForDownloadFilename(normalizedSuggestedFilename);
        if (!finalFilename) {
          return;
        }
      } else {
        finalFilename = await requestPredictionDownloadFilename(normalizedSuggestedFilename);
        if (!finalFilename) {
          return;
        }
      }
    ...
    if (!response.ok) {
      throw new Error("The prediction file could not be downloaded.");
    }
    ...
    console.error(error);
    window.alert(error instanceof Error ? error.message : "The prediction file could not be downloaded.");
  }
};
...
const hasRenderableMapData = () => {
  ...
  const activeDataset = datasetStore?.[activeDataViewTab];
  return Array.isArray(activeDataset?.geojson?.features) && activeDataset.geojson.features.length > 0;
};

const syncMapActionButtons = () => {
  const disabled = !hasRenderableMapData();
  [mapScreenshotButton, mapZoomInButton, mapZoomOutButton, mapToggleExpandButton]
    .filter(Boolean)
    .forEach((button) => {
...
const syncMapCanvasSizing = () => {
  if (tilesContainer) {
    if (mapFrameExpanded && mapFrame) {
      const scaleX = mapFrame.clientWidth / width;
      const scaleY = mapFrame.clientHeight / height;
      tilesContainer.style.width = `${width}px`;
      tilesContainer.style.height = `${height}px`;
      tilesContainer.style.transformOrigin = "top left";
      tilesContainer.style.transform = `scale(${scaleX}, ${scaleY})`;
    } else {
      tilesContainer.style.width = "";
      tilesContainer.style.height = "";
      tilesContainer.style.transformOrigin = "";
      tilesContainer.style.transform = "";
    }
  }
  if (mapFrameExpanded) {
  ...
  if (mapContext?.renderBase) {
    mapContext.renderBase(currentTransform);
  }
  syncMapActionButtons();
};

const syncMapViewportControls = () => {
  if (mapFrame) {
    mapFrame.classList.toggle("map-frame-expanded", mapFrameExpanded);
  }
  document.body.classList.toggle("map-expanded", mapFrameExpanded);
  if (mapToggleExpandButton) {
    mapToggleExpandButton.setAttribute("aria-label", mapFrameExpanded ? "Restore map size" : "Maximize map");
    mapToggleExpandButton.title = mapFrameExpanded ? "Restore map size" : "Maximize map";
  }
  syncMapCanvasSizing();
};

const getIsMapFrameFullscreen = () => document.fullscreenElement === mapFrame;

const setMapFrameExpanded = async (expanded) => {
  const nextExpanded = Boolean(expanded);
  if (mapFrame && typeof mapFrame.requestFullscreen === "function") {
    if (nextExpanded && !getIsMapFrameFullscreen()) {
      await mapFrame.requestFullscreen();
      return;
    }
    if (!nextExpanded && getIsMapFrameFullscreen() && typeof document.exitFullscreen === "function") {
      await document.exitFullscreen();
      return;
    }
  }
  mapFrameExpanded = nextExpanded;
  syncMapViewportControls();
};

const toggleMapFrameExpanded = async () => {
  await setMapFrameExpanded(!mapFrameExpanded);
};

const zoomMapBy = (scaleFactor) => {
  if (!zoomBehavior) {
    return;
  }
  svg
    .transition()
    .duration(220)
    .call(zoomBehavior.scaleBy, scaleFactor, [width / 2, height / 2]);
};

const inlineSvgComputedStyles = (sourceNode, targetNode) => {
  ...
  sourceChildren.forEach((childNode, index) => {
    inlineSvgComputedStyles(childNode, targetChildren[index]);
  });
};

const drawSvgOnCanvas = async (canvasContext, frameRect) => {
  const svgNode = svg.node();
  if (!svgNode) {
    return;
  }
  const svgClone = svgNode.cloneNode(true);
  inlineSvgComputedStyles(svgNode, svgClone);
...
const downloadMapScreenshot = async () => {
  if (!mapFrame || !hasRenderableMapData()) {
    return;
  }
  const frameRect = mapFrame.getBoundingClientRect();
  const canvas = document.createElement("canvas");
  canvas.width = Math.max(1, Math.round(frameRect.width));
  canvas.height = Math.max(1, Math.round(frameRect.height));
  const canvasContext = canvas.getContext("2d");
  if (!canvasContext) {
    throw new Error("The map screenshot could not be prepared.");
  }
  const backgroundStyle = getComputedStyle(mapFrame).backgroundColor;
  canvasContext.fillStyle = backgroundStyle && backgroundStyle !== "rgba(0, 0, 0, 0)" ? backgroundStyle : "#edf1eb";
  canvasContext.fillRect(0, 0, canvas.width, canvas.height);

  const tileImages = Array.from(tilesContainer?.querySelectorAll("img.tile") ?? []);
  ...
  await drawSvgOnCanvas(canvasContext, frameRect);
...
const isClearSelectionSingleGermplasmPrediction = () =>
...
const isMultiProfilePredictionResult = () =>
  activeDataViewTab === "prediction" &&
  !isClearSelectionSingleGermplasmPrediction() &&
  Boolean(datasetStore.prediction?.summary?.multi_profile_mode);

const getPredictionDistinctGermplasmNames = () => {
  const summaryNames = Array.isArray(datasetStore.prediction?.summary?.manual_grid_prediction_names)
    ? datasetStore.prediction.summary.manual_grid_prediction_names
        .map((name) => String(name ?? "").trim())
        .filter(Boolean)
    : [];
  if (summaryNames.length) {
    return Array.from(new Set(summaryNames));
  }
...
const normalizeWorkbookHeaderName = (value) =>
  String(value ?? "").replace(/\u00a0/g, " ").replace(/\s+/g, " ").trim();

const normalizeWorkbookHeaderKey = (value) => normalizeWorkbookHeaderName(value).toLowerCase();
const getXmlElementsByTagName = (node, tagName) => Array.from(node?.getElementsByTagNameNS?.("*", tagName) ?? []);
const getFirstXmlElementByTagName = (node, tagName) => getXmlElementsByTagName(node, tagName)[0] ?? null;

const getWorkbookColumnIndexFromReference = (reference) => {
...
const getWorkbookCellText = (cell, sharedStrings) => {
  const cellType = String(cell.getAttribute("t") ?? "").trim();
  if (cellType === "inlineStr") {
    return getXmlElementsByTagName(cell, "t").map((node) => node.textContent ?? "").join("").trim();
  }
  const valueNode = getFirstXmlElementByTagName(cell, "v");
  const rawValue = String(valueNode?.textContent ?? "").trim();
  if (cellType === "s") {
    const sharedIndex = Number.parseInt(rawValue, 10);
    return Number.isFinite(sharedIndex) ? String(sharedStrings[sharedIndex] ?? "").trim() : "";
  }
  return rawValue;
};

const resolveFirstWorkbookSheetPath = (workbookDocument, relationshipsDocument) => {
  const firstSheet = getFirstXmlElementByTagName(workbookDocument, "sheet");
  ...
  if (!relationshipId) {
    throw new Error("The workbook worksheet relationship could not be resolved.");
  }
  const relationships = getXmlElementsByTagName(relationshipsDocument, "Relationship");
  const relation = relationships.find((node) => String(node.getAttribute("Id") ?? "").trim() === relationshipId);
  const target = String(relation?.getAttribute("Target") ?? "").trim().replace(/^\/+/, "");
  if (!target) {
    throw new Error("The first worksheet path could not be resolved from the workbook.");
  }
  return target.startsWith("xl/") ? target : `xl/${target}`;
};

const readUploadedWorkbookHeadersAndRows = async (file) => {
  const cached = uploadedWorkbookPreviewCache.get(file);
  if (cached) {
    return cached;
  }
  const arrayBuffer = await file.arrayBuffer();
  const workbookArchive = await JSZip.loadAsync(arrayBuffer);
  const parser = new DOMParser();
  const workbookDocument = parser.parseFromString(
    await workbookArchive.file("xl/workbook.xml").async("text"),
    "application/xml",
  );
  const relationshipsDocument = parser.parseFromString(
    await workbookArchive.file("xl/_rels/workbook.xml.rels").async("text"),
    "application/xml",
  );
  const sharedStringsFile = workbookArchive.file("xl/sharedStrings.xml");
  const sharedStrings = sharedStringsFile
    ? Array.from(
      getXmlElementsByTagName(
        parser.parseFromString(await sharedStringsFile.async("text"), "application/xml"),
        "si",
      ),
    ).map((item) => getXmlElementsByTagName(item, "t").map((node) => node.textContent ?? "").join(""))
    : [];
  const sheetPath = resolveFirstWorkbookSheetPath(workbookDocument, relationshipsDocument);
  const sheetDocument = parser.parseFromString(
    await workbookArchive.file(sheetPath).async("text"),
    "application/xml",
  );
  const rows = getXmlElementsByTagName(sheetDocument, "row");
  if (!rows.length) {
    const emptyPayload = { headers: [], records: [] };
    uploadedWorkbookPreviewCache.set(file, emptyPayload);
    return emptyPayload;
  }
  const headerEntries = getXmlElementsByTagName(rows[0], "c")
    .map((cell) => {
      const index = getWorkbookColumnIndexFromReference(cell.getAttribute("r"));
      return index >= 0 ? [index, getWorkbookCellText(cell, sharedStrings)] : null;
    })
    .filter(Boolean)
    .sort((left, right) => left[0] - right[0]);
  const headers = headerEntries.map((entry) => normalizeWorkbookHeaderName(entry[1]));
  const headerIndexes = headerEntries.map((entry) => entry[0]);
  const records = rows.slice(1).map((row) => {
    const rowCells = new Map();
    getXmlElementsByTagName(row, "c").forEach((cell) => {
      const index = getWorkbookColumnIndexFromReference(cell.getAttribute("r"));
      if (index >= 0) {
        rowCells.set(index, getWorkbookCellText(cell, sharedStrings));
      }
    });
    const record = {};
    headers.forEach((header, headerOffset) => {
      record[header] = String(rowCells.get(headerIndexes[headerOffset]) ?? "").trim();
    });
    return record;
  });
  const payload = { headers, records };
  uploadedWorkbookPreviewCache.set(file, payload);
  return payload;
};

const resolveExistingWorkbookHeader = (headers, expectedHeader) => {
  const expectedKey = normalizeWorkbookHeaderKey(expectedHeader);
  if (!expectedKey) {
    return "";
  }
  return headers.find((header) => normalizeWorkbookHeaderKey(header) === expectedKey) ?? "";
};

const isExcludedPredictionIdField = (header) => {
  const normalizedHeader = normalizeWorkbookHeaderName(header);
  if (!normalizedHeader || normalizeWorkbookHeaderKey(normalizedHeader) === "name") {
    return true;
  }
  return /^DG\d+/i.test(normalizedHeader) || /^D01\d+/i.test(normalizedHeader);
};

const isMissingPredictionIdValue = (value) => {
  const normalizedValue = String(value ?? "").trim().toLowerCase();
  return !normalizedValue || ["null", "none", "nan"].includes(normalizedValue);
};

const collectDistinctWorkbookValues = (records, header) => {
  const values = [];
  const seen = new Set();
...
const buildPredictionGroupLabel = (name, idValue) => {
  const normalizedName = String(name ?? "").trim();
  const normalizedIdValue = String(idValue ?? "").trim();
  if (normalizedName && normalizedIdValue) {
    return `${normalizedName} - ${normalizedIdValue}`;
  }
  return normalizedName || normalizedIdValue;
};

const applyPredictionSelectedIdFallbacks = (records, selectedIdHeader) => {
  if (!selectedIdHeader) {
    return records;
  }
  ...
  const output = [];
  groupedByName.forEach((nameRecords) => {
    if (nameRecords.length <= 1) {
      output.push(...nameRecords);
      return;
    }
    const usedValues = new Set(
      nameRecords
        .map((record) => String(record?.[selectedIdHeader] ?? "").trim())
        .filter((value) => !isMissingPredictionIdValue(value)),
    );
    let nextFallback = 1;
    nameRecords.forEach((record) => {
      let value = String(record?.[selectedIdHeader] ?? "").trim();
      if (isMissingPredictionIdValue(value)) {
        while (usedValues.has(String(nextFallback))) {
          nextFallback += 1;
        }
        value = String(nextFallback);
        nextFallback += 1;
        record[selectedIdHeader] = value;
...
const buildUploadedWorkbookPredictionPreview = async (file, { selectedIdHeader = "" } = {}) => {
  const { headers, records } = await readUploadedWorkbookHeadersAndRows(file);
  if (!headers.length) {
    throw new Error("The uploaded prediction workbook is empty.");
  }
  const nameHeader = resolveExistingWorkbookHeader(headers, "Name");
  if (!nameHeader) {
    throw new Error("The uploaded workbook does not contain the required Name column.");
  }
  const farmHeader = resolveExistingWorkbookHeader(headers, "Farm");
  if (!farmHeader) {
    throw new Error("The uploaded workbook must contain the required Farm column.");
  }
  const requestedClearSelection = String(selectedIdHeader ?? "").trim() === clearSelectionIdSentinel;
  const resolvedSelectedIdHeader = requestedClearSelection
    ? ""
    : resolveExistingWorkbookHeader(headers, selectedIdHeader) || farmHeader;
  const annotatedRecords = applyPredictionSelectedIdFallbacks(
    records.map((record, index) => ({
      ...record,
      Name: String(record?.[nameHeader] ?? "").trim(),
      ...(resolvedSelectedIdHeader
        ? { [resolvedSelectedIdHeader]: String(record?.[resolvedSelectedIdHeader] ?? "").trim() }
        : {}),
      "Prediction Internal Row Id": `source-row-${index + 2}`,
    })),
    resolvedSelectedIdHeader,
  );
  ...
  annotatedRecords.forEach((record) => {
    ...
    const idValue = resolvedSelectedIdHeader ? String(record?.[resolvedSelectedIdHeader] ?? "").trim() : "";
    const label = buildPredictionGroupLabel(name, idValue);
    const group = groupedByLabel.get(label) ?? [];
    group.push(record);
    groupedByLabel.set(label, group);
  });
  const groupedByName = new Map();
  groupedByLabel.forEach((groupedRecords, label) => {
    const sampleRecord = groupedRecords[0] ?? {};
    const name = String(sampleRecord?.Name ?? "").trim();
    if (!name) {
      return;
    }
    const nameGroup = groupedByName.get(name) ?? [];
    nameGroup.push({
      key: label,
      idValue: String(sampleRecord?.[resolvedSelectedIdHeader] ?? "").trim(),
      records: groupedRecords,
    });
    groupedByName.set(name, nameGroup);
  });
  const orderedLabels = Array.from(groupedByName.keys()).sort((left, right) => left.localeCompare(right, undefined, { sensitivity: "base", numeric: true }));
  const profilesByLabel = {};
  const idValuesByLabel = {};
  orderedLabels.forEach((label) => {
    const nameGroups = groupedByName.get(label) ?? [];
    const groupedRecords = nameGroups.flatMap((entry) => entry.records);
    const name = label;
    const showRepCountInIdSummary = resolvedSelectedIdHeader && normalizeWorkbookHeaderKey(resolvedSelectedIdHeader) !== "farm";
    const idValues = nameGroups
      .map((entry) => String(entry.idValue ?? "").trim())
      .filter(Boolean);
    const idValue = idValues.join(" / ");
    const displayIdValue = nameGroups
      .map((entry) => {
        const normalizedIdValue = String(entry.idValue ?? "").trim();
        const repCount = Array.isArray(entry.records) ? entry.records.length : 0;
        if (!normalizedIdValue) {
          return "";
        }
        if (!showRepCountInIdSummary) {
          return normalizedIdValue;
        }
        return `${normalizedIdValue} (${repCount} rep${repCount === 1 ? "" : "s"})`;
      })
      .filter(Boolean)
      .join(" / ");
    const candidateHeaders = headers.filter((header) =>
      header
      && header !== nameHeader
      && (!resolvedSelectedIdHeader || header !== resolvedSelectedIdHeader)
      && !/^DG\d+$/i.test(header),
    );
    const varyingColumns = [];
    const profileKeyHeaders = [];
    candidateHeaders.forEach((header) => {
      const distinctValues = collectDistinctWorkbookValues(groupedRecords, header);
      if (distinctValues.length <= 1) {
        return;
      }
      varyingColumns.push({
        header,
        values: distinctValues.slice(0, 8),
        unique_count: distinctValues.length,
      });
      profileKeyHeaders.push(header);
    });
    const profileKeys = new Set();
    if (resolvedSelectedIdHeader && profileKeyHeaders.length) {
      groupedRecords.forEach((record) => {
        const profileKey = profileKeyHeaders
          .map((header) => `${header}=${String(record?.[header] ?? "").trim() || "Blank"}`)
          .join(" || ");
        if (profileKey) {
    ...
    profilesByLabel[label] = {
      name,
      id_value: idValue,
      display_label: buildPredictionGroupLabel(name, displayIdValue || idValue),
      row_count: groupedRecords.length,
      rep_count: groupedRecords.length,
      multi_record: groupedRecords.length > 1,
      profile_count: resolvedSelectedIdHeader ? Math.max(profileKeys.size, 1) : 1,
      multi_profile_mode: resolvedSelectedIdHeader ? profileKeys.size > 1 : false,
      varying_columns: varyingColumns,
      internal_row_ids: groupedRecords
        .map((record) => String(record?.["Prediction Internal Row Id"] ?? "").trim())
        .filter(Boolean),
    };
    idValuesByLabel[label] = idValues;
  });
  return {
    headers,
    id_field_candidates: headers.filter((header) => !isExcludedPredictionIdField(header)),
    germplasm_names: orderedLabels,
    germplasm_profiles_by_name: profilesByLabel,
    selected_id_header: resolvedSelectedIdHeader,
    germplasm_id_values_by_name: idValuesByLabel,
    count: orderedLabels.length,
  };
};

const normalizePredictionIdDisplayValue = (value) => {
...
const getPredictionSelectedIdValueForProperties = (properties) => {
  const selectedIdHeader = getPredictionSelectedIdHeader();
  if (!selectedIdHeader) {
    return "";
  }
  return normalizePredictionIdDisplayValue(properties?.[selectedIdHeader] ?? "");
};

const normalizePredictionCategoryLabel = (label) => {
  ...
  const idValue = normalizePredictionIdDisplayValue(parts.join(" - "));
  if (!name) {
    return idValue;
  }
  if (!idValue) {
...
const getPredictionSelectedIdValueForFeature = (feature) => {
  return getPredictionSelectedIdValueForProperties(feature?.properties);
};

const getPredictionNameAndIdLabelForFeature = (feature) => {
  const name = String(feature?.properties?.Name ?? "").trim();
  const idValue = getPredictionSelectedIdValueForFeature(feature);
  if (name && idValue) {
    return normalizePredictionCategoryLabel(`${name} - ${idValue}`);
  }
  return name || idValue;
};

const getPredictionCategoryLabelForFeature = (feature) => {
  const nameAndIdLabel = getPredictionNameAndIdLabelForFeature(feature);
  if (nameAndIdLabel && isCategoricalPredictionColorMode()) {
    return nameAndIdLabel;
  }
...
const getPredictionProfileDisplayLabel = (label) => {
  const normalizedLabel = normalizePredictionCategoryLabel(label);
  if (!normalizedLabel) {
    return "Profile";
  }
  if (getPredictionSelectedIdHeader() && normalizedLabel.includes(" - ")) {
...
const getSelectedPredictionNameIdLabels = () => {
  const selectedIdHeader = getPredictionSelectedIdHeader();
  if (!selectedIdHeader) {
    return [];
  }
  ...
  Array.from(selectedGermplasmNames)
    ...
    .forEach((selectedLabel) => {
      const metadata = availablePredictionSelectionGroupsByLabel?.[selectedLabel];
      const sourceName = String(metadata?.name ?? selectedLabel).trim();
      const idValues = Array.isArray(availablePredictionIdValuesByName?.[selectedLabel])
        ? availablePredictionIdValuesByName[selectedLabel]
            .map((value) => normalizePredictionIdDisplayValue(value))
            .filter(Boolean)
        : [];
      if (idValues.length > 0) {
        idValues.forEach((idValue) => {
          const label = normalizePredictionCategoryLabel(`${sourceName} - ${idValue}`);
      ...
      const displayLabel = String(metadata?.display_label ?? selectedLabel).trim();
      if (displayLabel) {
        if (!labels.includes(displayLabel)) {
          labels.push(normalizePredictionCategoryLabel(displayLabel));
        }
        return;
      }
    });
  return labels.sort((left, right) => left.localeCompare(right, undefined, { sensitivity: "base", numeric: true }));
};

const mergePredictionCategoryLabels = (...labelGroups) => {
  ...
  const hasNameIdPairs = labels.some((label) => label.includes(" - "));
  const filteredLabels = hasNameIdPairs
    ? labels.filter((label) => label.includes(" - "))
    : labels;
  return filteredLabels.sort((left, right) => left.localeCompare(right, undefined, { sensitivity: "base", numeric: true }));
};

const getPredictionProfileColorMap = () => {
  if (isCategoricalPredictionColorMode() && getPredictionSelectedIdHeader()) {
    ...
    const labels = mergePredictionCategoryLabels(featureLabels, getSelectedPredictionNameIdLabels());
    return new Map(
...
const getPredictionProfileColor = (label) => {
  const normalized = normalizePredictionCategoryLabel(label);
  if (!normalized) {
    return "#2e7d32";
  }
...
const clearDataset = async (tab) => {
  ...
  if (tab === "prediction") {
    manualGridOverlayComputationCache = null;
...
const buildFeatureCollectionSubset = (geojson, selectedNames, sourceName) => {
  ...
  const features = templateFeatures.flatMap((templateFeature, templateIndex) =>
    sourceFeatures.map((baseFeature, baseIndex) => {
      ...
      const selectedIdHeader = getPredictionSelectedIdHeader();
      if (selectedIdHeader && selectedIdHeader in templateProperties) {
        properties[selectedIdHeader] = templateProperties[selectedIdHeader];
      }
...
const filterPredictionGeojsonToSelectedNames = (geojson, selectedNames = []) => {
  ...
  const filteredFeatures = sourceFeatures.filter((feature) => {
    ...
    const nameAndIdLabel = featureName && selectedIdValue ? normalizePredictionCategoryLabel(`${featureName} - ${selectedIdValue}`) : "";
    return (
      normalizedSelectedNames.has(featureName) ||
      normalizedSelectedNames.has(projectedName) ||
      normalizedSelectedNames.has(nameAndIdLabel)
    );
  });
  return {
    ...geojson,
...
const resetTrainingResultDownload = () => {
  if (trainingResultDownloadPanel) {
    trainingResultDownloadPanel.hidden = true;
  }
  if (trainingResultDownloadLink) {
    trainingResultDownloadLink.href = "#";
    trainingResultDownloadLink.download = "";
  }
  if (trainingResultDownloadMessage) {
    trainingResultDownloadMessage.textContent =
      "Download the training prediction workbook generated for the map shown in Training.";
  }
};

const setTrainingResultDownload = (downloadUrl, filename = "ce_training_results.xlsx") => {
  const resolvedUrl = String(downloadUrl ?? "").trim();
  if (!trainingResultDownloadPanel || !trainingResultDownloadLink || !resolvedUrl) {
    resetTrainingResultDownload();
    return;
  }
  trainingResultDownloadLink.href = resolvedUrl;
  trainingResultDownloadLink.download = String(filename ?? "").trim() || "ce_training_results.xlsx";
  if (trainingResultDownloadMessage) {
    trainingResultDownloadMessage.textContent =
      "Download the training prediction workbook that backs the current Training map.";
  }
  trainingResultDownloadPanel.hidden = false;
};

const buildTrainingResultsDownloadUrlFromRunId = (runId) => {
  const normalizedRunId = String(runId ?? "").trim();
  if (!normalizedRunId) {
    return "";
  }
  return `/api/process-xlsx/selecction-properties/download?run_id=${encodeURIComponent(normalizedRunId)}&phase=training_results`;
};

const getTrainingResultDownloadRequest = () => {
  const latestRun = selectionPropertiesState.latestSummaryRun;
  const latestRunUrl = String(
    latestRun?.training_results_download_url
      ?? latestRun?.training_summary?.download_url
      ?? "",
  ).trim();
  const latestRunName = String(
    latestRun?.training_results_download_name
      ?? latestRun?.training_summary?.download_file_name
      ?? "",
  ).trim();
  if (latestRunUrl) {
    return {
      url: latestRunUrl,
      filename: latestRunName || "ce_training_results.xlsx",
    };
  }

  const latestRunIdUrl = buildTrainingResultsDownloadUrlFromRunId(latestRun?.run_id ?? latestRun?.job_id ?? "");
  if (latestRunIdUrl) {
    return {
      url: latestRunIdUrl,
      filename: latestRunName || "ce_training_results.xlsx",
  ...
  const trainingSummary = datasetStore.training?.summary ?? null;
  const trainingSummaryUrl = String(trainingSummary?.download_url ?? "").trim();
  const trainingSummaryName = String(trainingSummary?.download_file_name ?? "").trim();
  if (trainingSummaryUrl) {
    return {
      url: trainingSummaryUrl,
      filename: trainingSummaryName || "ce_training_results.xlsx",
    };
  }

  const linkUrl = String(trainingResultDownloadLink?.getAttribute("href") ?? "").trim();
  const linkName = String(trainingResultDownloadLink?.download ?? "").trim();
  if (!linkUrl || linkUrl === "#") {
    return null;
  }
  return {
    url: linkUrl,
    filename: linkName || "ce_training_results.xlsx",
  };
};

const triggerTrainingResultDownload = () => {
  if (!cePipelineDownloadEnabled) {
    return;
  }
  const downloadRequest = getTrainingResultDownloadRequest();
  if (!downloadRequest) {
    return;
  }
  triggerSelectionPropertiesDownload(downloadRequest.url, downloadRequest.filename);
};
...
const getOriginalPredictionIdentifierCandidates = (payload = {}) => {
  const payloadHeaders = new Set(
    (Array.isArray(payload?.headers) ? payload.headers : [])
      .map((header) => String(header ?? "").trim())
      .filter(Boolean),
  );
  const workspaceIdentifiers = Array.isArray(selectionPropertiesState.identifierColumns)
    ? selectionPropertiesState.identifierColumns
    : Array.isArray(predictionBridgeState?.summary?.identifier_columns)
      ? predictionBridgeState.summary.identifier_columns
      : [];
  return sortSelectionValues(
    workspaceIdentifiers
      .map((header) => String(header ?? "").trim())
      .filter((header) => header && payloadHeaders.has(header) && !isPredictionIdFieldExcluded(header)),
  );
};
...
const cancelPredictionIdSelection = async () => {
  resetPendingGermplasmSelection();
  if (fileInput) {
    fileInput.value = "";
  }
  ...
  await setLoadingState(
    0,
    "Load cancelled",
    "No workbook was loaded because the Id selection was cancelled.",
    false,
  );
};
...
const populateEmbeddedPredictionIdSelector = (options = [], selectedValue = "") => {
  if (!germplasmSelectionIdPicker || !germplasmSelectionIdSelect) {
    return;
  }
  const filteredOptionValues = sortSelectionValues(
    Array.from(
      new Set(
        options
          .map((value) => String(value ?? "").trim())
          .filter((value) => {
            ...
            return value
              && value !== clearSelectionIdSentinel
              && normalized !== "clear selection"
              && normalized !== "__clear_selection__";
          }),
      ),
    ),
  );
  ...
  populateSelectionListbox(
    germplasmSelectionIdSelect,
    normalizedOptions,
    normalizedOptions.length ? "Select column id" : "No fields available",
    normalizedSelectedValue,
    !normalizedOptions.length,
  );
};

const getSelectedPredictionRowIds = () => {
  const selectedRowIds = [];
  Array.from(selectedGermplasmNames)
    .map((value) => String(value ?? "").trim())
    .filter(Boolean)
    .forEach((label) => {
      const metadata = availablePredictionSelectionGroupsByLabel?.[label];
      const internalRowIds = Array.isArray(metadata?.internal_row_ids) ? metadata.internal_row_ids : [];
      internalRowIds
        .map((value) => String(value ?? "").trim())
        .filter(Boolean)
        .forEach((rowId) => {
...
const getSelectedPredictionSourceNames = () => {
  const selectedNames = [];
  Array.from(selectedGermplasmNames)
    .map((value) => String(value ?? "").trim())
    .filter(Boolean)
    .forEach((label) => {
      const metadata = availablePredictionSelectionGroupsByLabel?.[label];
      const sourceName = String(metadata?.name ?? label).trim();
      if (sourceName && !selectedNames.includes(sourceName)) {
        selectedNames.push(sourceName);
      }
    });
...
const openPredictionIdModal = (payload) => {
  ...
  const mode = String(payload?.prediction_id_modal_mode ?? predictionGermplasmSourceMode ?? "uploaded").trim();
  availablePredictionIdFields = mode === "original"
    ? getOriginalPredictionIdentifierCandidates(payload)
  ...
  if (predictionIdModalDescription) {
    predictionIdModalDescription.textContent = availablePredictionIdFields.length
      ? mode === "original"
        ? "Choose which workspace identifier should be used as the Id beside each germplasm in the original-data selection list."
        : "Choose the workbook field that should be shown as the Id beside each germplasm in the selection list."
      : mode === "original"
        ? "No workspace identifier fields are available for this original-data list."
  ...
  populateSelectionListbox(
    ...
    availablePredictionIdFields.length ? "Select id field" : "No fields available",
    "",
    !availablePredictionIdFields.length,
  );
...
const resetPendingGermplasmSelection = () => {
  ...
  selectedPredictionIdField = "";
  availablePredictionIdValuesByName = {};
  availablePredictionSelectionGroupsByLabel = {};
  pendingPredictionIdModalPayload = null;
  ...
  populateEmbeddedPredictionIdSelector([], "");
...
const renderGermplasmSelectionList = () => {
  ...
  availableGermplasmNames.forEach((name) => {
    ...
    const displayLabel = String(metadata?.display_label ?? name).trim() || name;
    const rowCount = Number(metadata?.row_count ?? metadata?.rep_count ?? 0);
    const hasSelectedIdField = Boolean(getPredictionSelectedIdHeader());
    const text = document.createElement("span");
    text.textContent = !hasSelectedIdField && rowCount > 1
      ? `${String(metadata?.name ?? name).trim() || name} (${rowCount} reps)`
      : displayLabel;
    label.title = Array.isArray(metadata?.varying_columns) && metadata.varying_columns.length
...
const settlePredictionDownloadRequest = (payload) => {
  if (!pendingPredictionDownloadRequest) {
    return;
  }
  const { resolve } = pendingPredictionDownloadRequest;
  pendingPredictionDownloadRequest = null;
  resolve(payload);
};

const closePredictionDownloadModal = ({ keepFocus = true } = {}) => {
  if (predictionDownloadModal) {
    predictionDownloadModal.hidden = true;
    predictionDownloadModal.setAttribute("inert", "");
    predictionDownloadModal.setAttribute("aria-hidden", "true");
  }
  if (predictionDownloadModalCard) {
    predictionDownloadModalCard.removeAttribute("role");
    predictionDownloadModalCard.removeAttribute("aria-modal");
  }
  if (predictionDownloadNameInput) {
    predictionDownloadNameInput.value = "";
  }
  if (predictionDownloadExtension) {
    predictionDownloadExtension.textContent = ".xlsx";
  }
  if (keepFocus && lastFocusedElement instanceof HTMLElement) {
    lastFocusedElement.focus();
  }
...
const confirmPredictionDownloadFromDialog = () => {
  const normalizedName = String(predictionDownloadNameInput?.value ?? "").trim();
  if (!normalizedName) {
    predictionDownloadNameInput?.focus();
    return;
  }
  closePredictionDownloadModal();
  settlePredictionDownloadRequest({ action: "download", value: normalizedName });
};

const cancelPredictionDownloadDialog = () => {
  closePredictionDownloadModal();
  settlePredictionDownloadRequest({ action: "cancel", value: "" });
};

const openPredictionDownloadModal = ({ suggestedName = "prediction_output", extension = ".xlsx" } = {}) =>
  new Promise((resolve) => {
    if (!predictionDownloadModal || !predictionDownloadNameInput) {
      resolve({ action: "download", value: suggestedName });
      return;
    }
    lastFocusedElement =
      document.activeElement instanceof HTMLElement ? document.activeElement : null;
    pendingPredictionDownloadRequest = { resolve };
    predictionDownloadModalDescription.textContent =
      "Enter the file name to download the High-Potential Sites prediction results.";
    predictionDownloadModalMeta.textContent =
      `File type: ${String(extension || ".xlsx").replace(/^\./, "").toUpperCase()} | The extension will be kept automatically if you do not type it.`;
    predictionDownloadNameInput.value = suggestedName;
    if (predictionDownloadExtension) {
      predictionDownloadExtension.textContent = extension || ".xlsx";
    }
    predictionDownloadModal.hidden = false;
    predictionDownloadModal.removeAttribute("inert");
    predictionDownloadModal.setAttribute("aria-hidden", "false");
    predictionDownloadModalCard.setAttribute("role", "dialog");
    predictionDownloadModalCard.setAttribute("aria-modal", "true");
    window.setTimeout(() => {
      predictionDownloadNameInput.focus();
      predictionDownloadNameInput.select();
    }, 0);
  });
...
const updateInlineScrollerControls = (scroller, prevButton, nextButton) => {
  if (!scroller || !prevButton || !nextButton) {
    return;
  }
  const maxScroll = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
  prevButton.disabled = scroller.scrollLeft <= 2;
  nextButton.disabled = scroller.scrollLeft >= maxScroll - 2 || maxScroll <= 0;
};

const scrollInlineScrollerByPage = (scroller, direction = 1) => {
  if (!scroller) {
    return;
  }
...
const startPipelineJob = async (
  ...
  if (selectedGermplasmRowIds.length) {
    headers["X-Selected-Germplasm-Row-Ids"] = JSON.stringify(selectedGermplasmRowIds);
  }
  ...
  if (
    ...
    climateScope === "regional_manual" &&
    usingBridgeSourceFile
  ) {
    headers["X-Selected-Germplasm-Selection-Mode"] =
      predictionGermplasmSourceMode === "original" ? "all_matches" : "first_match_only";
  }
...
const loadGermplasmOptions = async (file, { selectedIdHeader = "", mode = "uploaded" } = {}) => {
  const headers = {
    "Content-Type":
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    "X-File-Name": file.name,
...
const handleSelectionSummaryPlay = async () => {
  ...
  try {
    ...
    selectionPropertiesState.collapsed = true;
    setTrainingResultDownload(
      payload?.training_results_download_url,
      payload?.training_results_download_name ?? payload?.training_summary?.prediction_xlsx?.split('/').pop() ?? 'ce_training_results.xlsx',
    );
...
const initializeSelectionChoice = (element, config) => {
  const ChoicesCtor = window.Choices;
  if (typeof ChoicesCtor !== "function" || !element || selectionChoices.has(element)) {
    return;
  }
  const instance = new ChoicesCtor(element, {
    ...
    renderChoiceLimit: -1,
    renderSelectedChoices: config.renderSelectedChoices ?? "always",
  });
...
const initializeSelectionChoices = () => {
  ...
  selectionChoiceConfigs.forEach((config, element) => {
    initializeSelectionChoice(element, config);
  });
};
...
const requiredIdentifierColumns = ["Name", "Farm"];

const resolveSelectionColumns = (availableColumns = [], expectedColumns = []) => {
  const normalizedMap = new Map(
    availableColumns.map((column) => [String(column ?? "").trim().toLowerCase(), String(column ?? "").trim()]),
  );
  return expectedColumns
    .map((column) => normalizedMap.get(String(column ?? "").trim().toLowerCase()) || "")
    .filter(Boolean);
};

const getRequiredIdentifierColumns = (availableColumns = getAvailableIdentifierColumns()) =>
  resolveSelectionColumns(availableColumns, requiredIdentifierColumns);
...
const maybeAutoSelectIdentifierColumns = () => {
  ...
  const autoSelectedColumns = [
    ...getRequiredIdentifierColumns(availableIdentifierColumns),
    ...preferredIdentifierColumns.filter((column) => availableIdentifierColumns.includes(column)),
  ];
...
const recalculateSelectionPropertiesGroups = ({ resetClassificationConfirmation = false } = {}) => {
  const availableIdentifierColumns = getAvailableIdentifierColumns();
  const requiredIdentifiers = getRequiredIdentifierColumns(availableIdentifierColumns);
  ...
  selectionPropertiesState.identifierColumns = sortIdentifierSelectionValues([
    ...requiredIdentifiers,
    ...selectionPropertiesState.identifierColumns,
  ]);
  if (
...
const setIdentifierColumnsSelection = (columns = [], highlightedColumn = "", scrollTop = 0) => {
  ...
  const requiredIdentifiers = getRequiredIdentifierColumns();
  selectionPropertiesState.identifierColumns = sortIdentifierSelectionValues([
    ...requiredIdentifiers,
    ...columns,
  ]);
...
const applyIdentifierColumnsSelection = (columns = []) => {
  const requiredIdentifiers = getRequiredIdentifierColumns();
  const normalizedColumns = sortIdentifierSelectionValues([
    ...requiredIdentifiers,
    ...columns,
  ]);
...
const renderSelectionWorkbench = () => {
  ...
  renderSelectColumnm({
    ...
    lockedColumns: getRequiredIdentifierColumns(availableIdentifierColumns),
...
const populateSelectionListbox = (element, options, placeholder, selectedValue = "", disabled = false) => {
  ...
  const existingChoicesInstance = selectionChoices.get(element);
  if (existingChoicesInstance) {
    existingChoicesInstance.destroy();
    selectionChoices.delete(element);
  }
  element.innerHTML = "";
  const placeholderOption = document.createElement("option");
  placeholderOption.value = "";
  ...
  placeholderOption.selected = !allowedValues.has(normalizedSelectedValue);
  ...
  const selectionConfig = selectionChoiceConfigs.get(element);
  if (selectionConfig) {
    initializeSelectionChoice(element, selectionConfig);
    const rebuiltChoicesInstance = selectionChoices.get(element);
    if (rebuiltChoicesInstance) {
      if (allowedValues.has(normalizedSelectedValue)) {
        rebuiltChoicesInstance.setChoiceByValue(normalizedSelectedValue);
      }
      if (disabled) {
        rebuiltChoicesInstance.disable();
      } else {
        rebuiltChoicesInstance.enable();
      }
    }
    element.size = 1;
  }
...
const loadPredictionGermplasmList = async (file, { sourceLabel = "workbook", mode = "uploaded" } = {}) => {
  ...
  let payload = await loadGermplasmOptions(file, { mode });
  if (mode === "uploaded" || mode === "original") {
    payload = await buildUploadedWorkbookPredictionPreview(file, {
      selectedIdHeader: "Farm",
    });
  }
  ...
  availableGermplasmProfilesByName = payload?.germplasm_profiles_by_name && typeof payload.germplasm_profiles_by_name === "object"
    ...
    : {};
  availablePredictionSelectionGroupsByLabel = availableGermplasmProfilesByName;
  ...
  availablePredictionIdValuesByName = payload?.germplasm_id_values_by_name && typeof payload.germplasm_id_values_by_name === "object"
    ...
    : {};
  selectedGermplasmNames = new Set();
  populateEmbeddedPredictionIdSelector(availablePredictionIdFields, selectedPredictionIdField || clearSelectionIdSentinel);
  ...
  if (germplasmSelectionHelp) {
    germplasmSelectionHelp.textContent =
      mode === "original"
        ? `${payload.count ?? availableGermplasmNames.length} distinct germplasm names were loaded from ${payload.source_name ?? sourceLabel}. The original-data list uses Name + Farm by default, and Clear Selection will average all reps of the same germplasm before drawing the base prediction grid.`
  ...
  await setLoadingState(
    ...
    "Ready to run",
    mode === "original"
      ? "Choose one or more germplasms from the original-data list. With Clear Selection, repeated rows are averaged per germplasm before the base prediction grid is rendered with the UI forecast dates."
      : "Choose one or more Name + Id combinations from the list and then click Run to start the prediction flow using the workbook planting and harvesting dates.",
    false,
  );
};

const applyPredictionIdFieldSelection = async (selectedHeaderOverride = "") => {
  const selectedHeader = String(
    selectedHeaderOverride
    || germplasmSelectionIdSelect?.value
    || predictionIdColumnSelect?.value
    || "",
  ).trim();
  ...
  await setLoadingState(
    ...
    "Loading id field",
    predictionGermplasmSourceMode === "original"
      ? `Reading ${selectedHeader} from the original workspace workbook to annotate the germplasm list.`
      : `Reading ${selectedHeader} from the uploaded workbook to annotate the germplasm list.`,
    true,
  );
  const payload = predictionGermplasmSourceMode === "uploaded" || predictionGermplasmSourceMode === "original"
    ? await buildUploadedWorkbookPredictionPreview(pendingSavedModelFile, {
      selectedIdHeader: selectedHeader,
    })
    : await loadGermplasmOptions(pendingSavedModelFile, {
      selectedIdHeader: selectedHeader,
  ...
  await setLoadingState(
    ...
    "Ready to run",
    predictionGermplasmSourceMode === "original"
      ? "Choose one or more germplasms from the refreshed original-data list. With Clear Selection, repeated rows are averaged per germplasm before the base prediction grid is rendered."
      : "Choose one or more Name + Id combinations from the refreshed list and then click Run to start the prediction flow using the workbook planting and harvesting dates.",
    false,
  );
};
...
const SUMMARY_LOG_DESCRIPTIONS = {
  ...
  "Climate-prep source rows": "Rows passed into the climate-preparation workbook before climate joins and window calculations are written back.",
  "Rows kept for climate prep": "Rows that remained available after ce quality filters and were kept for the climate-preparation workbook.",
  "Climate-enriched output rows": "Rows produced after climate enrichment finished and the output workbook was generated.",
  ...
  "Missing required Initial Settings": "Rows removed because one or more required Initial Settings fields were missing before ce processing.",
  "Invalid target value": "Rows removed because the target variable could not be parsed into a valid numeric value.",
  "Invalid initial setting dates": "Rows removed because planting or harvesting dates were invalid or unusable.",
  "Water or lake point": "Rows removed because the coordinates fell on water or a lake instead of agricultural land.",
  ...
  "Climate cache hits": "Climate queries resolved from the local cache without calling the remote source again.",
  "Climate fresh queries": "New climate queries executed during this run because the cache did not already contain them.",
  "Soil skipped": "Whether the soil enrichment stage was intentionally skipped for this run.",
};
...
const createSummaryLogSection = (title, metrics, toneByKey = {}, description = "") => {
...
const renderSummaryLog = (summary = getActiveDatasetSummary()) => {
  ...
  if (cePipelineLog) {
    const overviewSection = createSummaryLogSection("Overview", {
      ...
      "Rows kept for climate prep": cePipelineLog?.overview?.phase02_kept_rows ?? 0,
      "Climate-enriched output rows": cePipelineLog?.overview?.phase03_output_rows ?? 0,
      "Climate unique queries": cePipelineLog?.overview?.nasa_unique_queries ?? 0,
    }, {}, "High-level ce pipeline counts from input through climate enrichment output.");
    const qualitySection = createSummaryLogSection(
      ...
      "Rows excluded before climate processing because required values, dates, target values, or land coordinates were invalid.",
    );
    ...
    }, {}, "Operational counts for climate and soil enrichment once rows reached the ce climate stage.");
  ...
  const overviewSection = createSummaryLogSection("Overview", {
    ...
    "Climate-prep source rows": preprocessLog?.overview?.phase02_source_rows ?? 0,
    "Climate unique queries": preprocessLog?.overview?.nasa_unique_queries ?? 0,
  }, {}, "High-level preprocess counts from raw input through the rows prepared for climate enrichment.");
  const qualitySection = createSummaryLogSection(
    "Quality Filters",
    {
    ...
    "Rows updated or excluded by preprocess naming, yield, rank, and coordinate quality rules.",
  );
  const climateSection = createSummaryLogSection("Climate Stage", {
  ...
  }, {}, "Rows that entered the climate stage, how many matched outputs, and how many were expanded across localities or projections.");
...
const setActivePredictionProfileHighlight = (profileLabel = "") => {
  activePredictionProfileHighlight = normalizePredictionCategoryLabel(profileLabel);
};

const getCheckedPredictionProfileLabels = () =>
  new Set(
    Array.from(checkedPredictionProfileLabels)
      .map((label) => normalizePredictionCategoryLabel(label))
      .filter(Boolean),
  );

const setCheckedPredictionProfileLabels = (labels) => {
  checkedPredictionProfileLabels = new Set(
    Array.from(labels ?? [])
      .map((label) => normalizePredictionCategoryLabel(label))
      .filter(Boolean),
  );
...
const isPredictionProfileChecked = (profileLabel) =>
  getCheckedPredictionProfileLabels().has(normalizePredictionCategoryLabel(profileLabel));

const applyCheckedPredictionProfiles = ({ markTouched = true } = {}) => {
  if (markTouched) {
    predictionProfileSelectionTouched = true;
  }
  const checkedLabels = getCheckedPredictionProfileLabels();
  if (checkedLabels.size === 1) {
    setActivePredictionProfileHighlight(Array.from(checkedLabels)[0]);
  } else {
    setActivePredictionProfileHighlight("");
  }
  updateProfileHighlightState();
  renderDetails();
  if (activeDataViewTab === "prediction" && isCategoricalPredictionColorMode()) {
    syncPredictionColorHelpState();
  } else {
    renderColorHelp();
  }
};

const runPredictionProfileInteractionWithLoading = async (action) => {
  if (typeof action !== "function") {
    return;
  }
  if (predictionProfileInteractionInProgress) {
    return;
  }
  if (!(activeDataViewTab === "prediction" && isCategoricalPredictionColorMode())) {
  ...
  predictionProfileInteractionInProgress = true;
  try {
    await Promise.resolve(action());
    const checkedLabels = getCheckedPredictionProfileLabels();
    ...
    predictionProfileInteractionInProgress = false;
  }
};

const togglePredictionProfileChecked = (profileLabel) => {
  const normalizedLabel = normalizePredictionCategoryLabel(profileLabel);
  if (!normalizedLabel) {
    return;
  }
  const nextCheckedLabels = getCheckedPredictionProfileLabels();
  if (nextCheckedLabels.has(normalizedLabel)) {
    nextCheckedLabels.delete(normalizedLabel);
  } else {
    nextCheckedLabels.add(normalizedLabel);
  }
  setCheckedPredictionProfileLabels(nextCheckedLabels);
  applyCheckedPredictionProfiles();
};
...
const updateProfileHighlightState = () => {
  ...
  const checkedLabels = getCheckedPredictionProfileLabels();
  const useCheckedLabels = predictionProfileSelectionTouched && checkedLabels.size > 0;
  if (mapContext?.markerGroups) {
    mapContext.markerGroups
      .classed("profile-highlighted", (feature) =>
        useCheckedLabels
          ? checkedLabels.has(getPredictionCategoryLabelForFeature(feature))
...
const getPredictionCategoryDominanceRanking = () => {
  const summaries = getAvailablePredictionProfileSummaries();
  const labels = summaries
    .map((summary) => normalizePredictionCategoryLabel(summary?.label))
    .filter(Boolean);
  if (!labels.length) {
    return [];
  }
  const rankingMap = new Map(labels.map((label) => [label, { label, wins: 0, mean: Number.NEGATIVE_INFINITY }]));
  const predictionFeatures = loadedFeatures.length
    ? loadedFeatures
    : Array.isArray(datasetStore.prediction?.geojson?.features)
      ? datasetStore.prediction.geojson.features
      : [];

  if (isManualGridPredictionResult()) {
    const gridPayload = getActiveManualGridPayload();
    const gridCells = Array.isArray(gridPayload?.cells) ? gridPayload.cells : [];
    const overlayState = getManualGridOverlayComputationState(gridCells);
    const dominanceLookupKey = "__dominance__";
    if (!overlayState.winningFeatureLookups.has(dominanceLookupKey)) {
      overlayState.winningFeatureLookups.set(
        dominanceLookupKey,
        buildManualGridWinningFeatureLookupFromEntries(overlayState.featureEntries),
      );
    }
    const winners = overlayState.winningFeatureLookups.get(dominanceLookupKey) ?? new Map();
    winners.forEach((feature) => {
      const label = normalizePredictionCategoryLabel(getPredictionCategoryLabelForFeature(feature));
      if (!rankingMap.has(label)) {
        rankingMap.set(label, { label, wins: 0, mean: Number.NEGATIVE_INFINITY });
      }
      rankingMap.get(label).wins += 1;
    });
  }
  ...
  groupedValues.forEach((values, label) => {
    ...
    rankingMap.get(label).mean = Number.isFinite(mean) ? mean : Number.NEGATIVE_INFINITY;
  });
...
const getDominantPredictionProfileLabel = () => {
  const ranking = getPredictionCategoryDominanceRanking();
  return ranking.length ? ranking[0].label : "";
};
...
const getFirstFeatureForProfileLabel = (profileLabel) => {
  const normalizedLabel = normalizePredictionCategoryLabel(profileLabel);
  if (!normalizedLabel) {
    return null;
  }
  const candidateFeatures = loadedFeatures.length
    ...
    : Array.isArray(datasetStore.prediction?.geojson?.features)
      ? datasetStore.prediction.geojson.features
      : [];
  return candidateFeatures.find((feature) =>
    normalizePredictionCategoryLabel(getPredictionCategoryLabelForFeature(feature)) === normalizedLabel,
  ) ?? null;
};
...
const showPredictionCategoryOnMap = (profileLabel) => {
  const normalizedLabel = normalizePredictionCategoryLabel(profileLabel);
  if (!normalizedLabel) {
    return;
  }
  setCheckedPredictionProfileLabels([normalizedLabel]);
  predictionProfileSelectionTouched = true;
  setActivePredictionProfileHighlight(normalizedLabel);
  const matchingFeature = getFirstFeatureForProfileLabel(normalizedLabel);
  if (matchingFeature) {
    selectedFeature = matchingFeature;
    currentPage = 0;
    if (mapContext?.markerGroups) {
      mapContext.markerGroups.classed("active", (datum) => datum === matchingFeature);
    }
  }
  updateProfileHighlightState();
  if (isManualGridPredictionResult() && mapContext?.renderBase) {
    mapContext.renderBase(currentTransform);
  }
  renderDetails();
  if (activeDataViewTab === "prediction" && isCategoricalPredictionColorMode()) {
...
const createColorHelpItem = ({
  ...
  if (predictionProfileLabel) {
    item.dataset.predictionProfileLabel = normalizePredictionCategoryLabel(predictionProfileLabel);
  }
  if (isActive) {
  ...
  stack.className = "color-help-stack";

  const check = document.createElement("span");
  check.className = "color-help-check";
  check.textContent = "✓";
  if (!showCheck) {
    check.hidden = true;
  }
  ...
  stack.append(check, swatch);
  item.append(stack);
  return item;
};

const syncPredictionColorHelpState = () => {
  if (!colorHelpList) {
    return;
  }
  const checkedLabels = getCheckedPredictionProfileLabels();
  colorHelpList.querySelectorAll(".color-help-item[data-prediction-profile-label]").forEach((item) => {
    const normalizedLabel = normalizePredictionCategoryLabel(item.dataset.predictionProfileLabel ?? "");
    const isActive = normalizedLabel ? checkedLabels.has(normalizedLabel) : false;
...
const buildMultiProfileColorHelpItems = () => {
  ...
  const ranking = getPredictionCategoryDominanceRanking();
  const rankedLabels = ranking.map((item) => item.label);
  const profilesByLabel = new Map(
    profiles.map((profile) => [normalizePredictionCategoryLabel(profile?.label), profile]),
  );
  const orderedProfiles = [
    ...rankedLabels.map((label) => profilesByLabel.get(label)).filter(Boolean),
    ...profiles.filter((profile) => !rankedLabels.includes(normalizePredictionCategoryLabel(profile?.label))),
  ];
  return orderedProfiles.map((profile) => ({
    ...
    tooltip: `Show ${getPredictionSelectedIdHeader() ? "germplasm" : isMultiGermplasmPredictionResult() ? "germplasm" : "profile"}: ${getPredictionProfileDisplayLabel(profile?.label)}`,
    isActive: isPredictionProfileChecked(profile?.label),
    showCheck: true,
    predictionProfileLabel: profile?.label,
    onClick: () => {
      const label = String(profile?.label ?? "").trim();
      runPredictionProfileInteractionWithLoading(() => {
        togglePredictionProfileChecked(label);
      }).catch((error) => {
        console.error(error);
      });
    },
...
const renderColorHelp = () => {
  ...
  if (activeDataViewTab === "training" && datasetStore.training?.geojson) {
    ...
    showYieldScale = !isCategoricalPredictionColorMode();
  }

  colorHelpPanel.hidden = items.length === 0;
  colorHelpList.replaceChildren(...items.map(createColorHelpItem));
  if (colorHelpScale) {
    colorHelpScale.hidden = !showYieldScale || items.length === 0;
    const showQuartileMarkers =
      (activeDataViewTab === "prediction" && !isCategoricalPredictionColorMode())
      || activeDataViewTab === "training";
    colorHelpScale
      .querySelectorAll(".color-help-scale-marker")
      .forEach((marker) => {
        marker.hidden = !showQuartileMarkers;
      });
  }
};
...
const getCurrentWorkspaceIdentifierKeys = () => {
  const selectionIdentifiers = Array.isArray(selectionPropertiesState.identifierColumns)
    ? selectionPropertiesState.identifierColumns
        .map((value) => String(value ?? "").trim())
        .filter(Boolean)
    : [];
  if (selectionIdentifiers.length) {
    return selectionIdentifiers;
  }

  const activeSummary = datasetStore[activeDataViewTab]?.summary;
  const activeSummaryIdentifiers = Array.isArray(activeSummary?.divisions?.germplams_identifiers)
    ? activeSummary.divisions.germplams_identifiers
        .map((value) => String(value ?? "").trim())
        .filter(Boolean)
    : [];
  if (activeSummaryIdentifiers.length) {
    return activeSummaryIdentifiers;
  }

  const originalSummary = datasetStore.original?.summary;
  const originalSummaryIdentifiers = Array.isArray(originalSummary?.divisions?.germplams_identifiers)
    ? originalSummary.divisions.germplams_identifiers
        .map((value) => String(value ?? "").trim())
        .filter(Boolean)
    : [];
  if (originalSummaryIdentifiers.length) {
    return originalSummaryIdentifiers;
  }

  const bridgeIdentifiers = Array.isArray(predictionBridgeState?.summary?.identifier_columns)
    ? predictionBridgeState.summary.identifier_columns
        .map((value) => String(value ?? "").trim())
        .filter(Boolean)
    : [];
  return bridgeIdentifiers.length ? bridgeIdentifiers : germplasmAttributeKeys;
};
...
const getPagedAttributes = (feature) => {
  const identifierKeys = new Set(getCurrentWorkspaceIdentifierKeys());
...
const syncAppConfig = async () => {
  try {
    ...
    cePipelineDownloadEnabled = payload.ce_pipeline_download_enabled !== false;
...
const renderDetails = () => {
  ...
  const selectedTitleIdValue = getPredictionSelectedIdValueForProperties(selectedFeature.properties);

  detailsTitle.textContent =
    name || selectedTitleIdValue
      ? [name || "No Name", selectedTitleIdValue].filter(Boolean).join(" - ")
  ...
  const selectedIdValue = getPredictionSelectedIdValueForProperties(selectedFeature.properties);
  if (selectedIdValue) {
    metaChips.push(`ID: ${formatAttributeValue(selectedIdValue)}`);
  }
  ...
  germplasmList.innerHTML = "";
  getCurrentWorkspaceIdentifierKeys().forEach((key) => {
    const value =
      isDefaultBridgeSavedModelMode() &&
...
const buildManualGridGroupedHeatSamples = (
  heatSamples,
  transform,
  paddingPx = 0,
  groupKeyResolver = (sample) => sample?.id,
) => {
  const groupedPaths = new Map();
  (heatSamples ?? []).forEach((sample) => {
    const groupKey = String(groupKeyResolver(sample) ?? "").trim();
    if (!groupKey) {
      return;
    }
    const pathSegment = buildManualGridScreenRectPath(sample, transform, paddingPx);
    groupedPaths.set(groupKey, `${groupedPaths.get(groupKey) ?? ""}${pathSegment}`);
  });
  return Array.from(groupedPaths.entries()).map(([groupKey, path]) => ({
    id: groupKey,
    groupKey,
    path,
  }));
...
const getManualGridInterpolatedSurface = () => {
  const surface = datasetStore.prediction?.summary?.manual_grid_interpolated_surface;
  return surface && typeof surface === "object" ? surface : null;
};

const getManualGridBackendOverlayBase = () => {
  const overlayBase = datasetStore.prediction?.summary?.manual_grid_overlay_base;
  return overlayBase && typeof overlayBase === "object" ? overlayBase : null;
};
...
const buildManualGridPredictedAggregation = (gridCells) => {
  const predictionFeatures =
    activeDataViewTab === "prediction" ? datasetStore.prediction?.geojson?.features ?? [] : [];
  const normalizedGridCells = (gridCells ?? []).map(normalizeManualGridCell);
  ...
  predictionFeatures.forEach((feature) => {
    ...
    const explicitReplicationCount = asFloat(
      feature?.properties?.["Prediction cell replication count"]
        ?? feature?.properties?.["Prediction cell row count"],
    );
    const nextCount = explicitReplicationCount !== null
      ? Math.max(1, Math.round(explicitReplicationCount))
      : (replicationCountLookup.get(gridCellId) ?? 0) + 1;
    replicationCountLookup.set(
      gridCellId,
      Math.max(replicationCountLookup.get(gridCellId) ?? 0, nextCount),
    );
  });

  const predictedLookup = new Map();
  valuesByCell.forEach((values, gridCellId) => {
    predictedLookup.set(gridCellId, d3.mean(values));
    if (!replicationCountLookup.has(gridCellId)) {
      replicationCountLookup.set(gridCellId, values.length);
    }
  });

  return {
    predictedLookup,
    replicationCountLookup,
    hasReplicatedCells: Array.from(replicationCountLookup.values()).some((count) => count > 1),
  };
};

const isSingleGermplasmNumericManualGrid = (predictedLookup) =>
  isManualGridPredictionResult() &&
  !isCategoricalPredictionColorMode() &&
  getPredictionDistinctGermplasmNames().length <= 1 &&
  predictedLookup instanceof Map &&
  predictedLookup.size > 0;

const buildManualGridWinningFeatureLookup = (gridCells, allowedLabels = null) => {
  const predictionFeatures =
    activeDataViewTab === "prediction" ? datasetStore.prediction?.geojson?.features ?? [] : [];
  const normalizedGridCells = (gridCells ?? []).map(normalizeManualGridCell);
  const normalizedAllowedLabels = allowedLabels instanceof Set
    ? new Set(Array.from(allowedLabels).map((label) => normalizePredictionCategoryLabel(label)).filter(Boolean))
    : null;
  const winners = new Map();
  predictionFeatures.forEach((feature) => {
    if (normalizedAllowedLabels?.size) {
      const featureLabel = normalizePredictionCategoryLabel(getPredictionCategoryLabelForFeature(feature));
      if (!normalizedAllowedLabels.has(featureLabel)) {
        return;
      }
    }
    const gridCellId = findManualGridCellIdForFeature(feature, normalizedGridCells);
...
const buildManualGridHeatSamples = (gridCells, predictedLookup, options = {}) => {
  const normalizedGridCells = (gridCells ?? []).map(normalizeManualGridCell);
  ...
  const samples = [];
  const subdivisionsPerCell = Number(options?.subdivisionsPerCell) > 0
    ? Number(options.subdivisionsPerCell)
    : 10;
  const weightPower = Number(options?.weightPower) > 0
    ? Number(options.weightPower)
    : 2;
  const neighborCount = Number(options?.neighborCount) > 0
    ? Math.max(1, Math.round(Number(options.neighborCount)))
    : 8;
  valuedCells.forEach((cell) => {
    ...
    for (let row = 0; row < subdivisionsPerCell; row += 1) {
      for (let column = 0; column < subdivisionsPerCell; column += 1) {
        ...
        const nearestCells = valuedCells
          .map((valuedCell) => ({
            valuedCell,
            distance: Math.hypot(
        ...
        let totalWeight = 0;
        nearestCells.forEach(({ valuedCell, distance }) => {
          const weight = 1 / distance ** weightPower;
...
const MANUAL_GRID_LIGHT_SMOOTH_FILTER_ID = "manual-grid-light-smooth-filter";
const MANUAL_GRID_REPLICATED_SMOOTH_FILTER_ID = "manual-grid-replicated-smooth-filter";
...
const MANUAL_GRID_REPLICATED_HEAT_OPACITY = 0.5;
const MANUAL_GRID_REPLICATED_SUBDIVISIONS = 18;
const MANUAL_GRID_REPLICATED_WEIGHT_POWER = 1.15;
const MANUAL_GRID_REPLICATED_NEIGHBOR_COUNT = 12;
const MANUAL_GRID_TARGET_INTERPOLATED_SAMPLE_BUDGET = 30000;
const MANUAL_GRID_MIN_CLIENT_INTERPOLATION_SUBDIVISIONS = 3;

const buildManualGridDirectHeatSamples = (gridCells, predictedLookup) =>
  (gridCells ?? []).map((cell) => ({
    id: String(cell.grid_cell_id ?? ""),
    longitudeMin: cell.longitude_min,
    longitudeMax: cell.longitude_max,
...
const resetManualGridLayerTransform = () => {
  if (!mapContext) {
    return;
  }
  mapContext.manualGridHeatLayer?.attr("transform", null);
  mapContext.manualGridCellsLayer?.attr("transform", null);
  mapContext.manualGridMarkersLayer?.attr("transform", null);
};

const buildManualGridDeltaTransform = (nextTransform, baseTransform = manualGridLastRenderedTransform) => {
  const baseScale = Number(baseTransform?.k) > 0 ? Number(baseTransform.k) : 1;
  const nextScale = Number(nextTransform?.k) > 0 ? Number(nextTransform.k) : 1;
  const scaleRatio = nextScale / baseScale;
  const translateX = Number(nextTransform?.x ?? 0) - Number(baseTransform?.x ?? 0) * scaleRatio;
  const translateY = Number(nextTransform?.y ?? 0) - Number(baseTransform?.y ?? 0) * scaleRatio;
  return `translate(${translateX},${translateY}) scale(${scaleRatio})`;
};

const applyManualGridOverlayZoomTransform = (transform) => {
  if (!mapContext?.manualGridHeatLayer || !mapContext?.manualGridCellsLayer || !mapContext?.manualGridMarkersLayer) {
    return false;
  }
  const hasRenderedOverlay =
    mapContext.manualGridHeatLayer.selectAll("path").size() > 0 ||
    mapContext.manualGridCellsLayer.selectAll("path").size() > 0 ||
    mapContext.manualGridMarkersLayer.selectAll("circle").size() > 0;
  if (!hasRenderedOverlay) {
    return false;
  }
  const deltaTransform = buildManualGridDeltaTransform(transform);
  mapContext.manualGridHeatLayer.attr("transform", deltaTransform);
  mapContext.manualGridCellsLayer.attr("transform", deltaTransform);
  mapContext.manualGridMarkersLayer.attr("transform", deltaTransform);
  return true;
};

const buildManualGridWinningFeatureLookupFromEntries = (featureEntries, allowedLabels = null) => {
  const normalizedAllowedLabels = allowedLabels instanceof Set
    ? new Set(Array.from(allowedLabels).map((label) => normalizePredictionCategoryLabel(label)).filter(Boolean))
    : null;
  const winners = new Map();
  featureEntries.forEach((entry) => {
    ...
    if (normalizedAllowedLabels?.size && !normalizedAllowedLabels.has(entry.featureLabel)) {
      return;
    }
    if (entry.predictedValue === null) {
      if (!winners.has(entry.gridCellId)) {
        winners.set(entry.gridCellId, { feature: entry.feature, predictedValue: Number.NEGATIVE_INFINITY });
      }
      return;
    }
    const current = winners.get(entry.gridCellId);
    if (!current || entry.predictedValue > current.predictedValue) {
      winners.set(entry.gridCellId, { feature: entry.feature, predictedValue: entry.predictedValue });
    }
  });
  return new Map(Array.from(winners.entries()).map(([gridCellId, payload]) => [gridCellId, payload.feature]));
};

const getManualGridClientInterpolationSubdivisions = (gridCells, backendInterpolatedSamples = []) => {
  if (backendInterpolatedSamples.length) {
    return 0;
  }
  const gridCellCount = Array.isArray(gridCells) ? gridCells.length : 0;
  if (gridCellCount <= 0) {
    return 0;
  }
  const adaptiveSubdivisions = Math.floor(
    Math.sqrt(MANUAL_GRID_TARGET_INTERPOLATED_SAMPLE_BUDGET / gridCellCount),
  );
  return Math.max(
    MANUAL_GRID_MIN_CLIENT_INTERPOLATION_SUBDIVISIONS,
    Math.min(MANUAL_GRID_REPLICATED_SUBDIVISIONS, adaptiveSubdivisions),
  );
};
...
const getManualGridOverlayComputationState = (gridCells) => {
  ...
  const featureEntries = predictionFeatures.map((feature) => {
    const explicitGridCellId = String(feature?.properties?.["Forecast grid cell id"] ?? "").trim();
    const gridCellId = explicitGridCellId || findManualGridCellIdForFeature(feature, normalizedGridCells);
    return {
      feature,
  ...
  let predictedLookup = null;
  if (backendOverlayBase?.mode === "numeric" && backendOverlayBase.predicted_lookup && typeof backendOverlayBase.predicted_lookup === "object") {
    predictedLookup = new Map(
      Object.entries(backendOverlayBase.predicted_lookup)
        .map(([gridCellId, value]) => [String(gridCellId), asFloat(value)])
        .filter(([, value]) => value !== null),
    );
  } else {
    const valuesByCell = new Map();
    featureEntries.forEach((entry) => {
      if (!entry.gridCellId || entry.predictedValue === null) {
        return;
      }
      if (!valuesByCell.has(entry.gridCellId)) {
        valuesByCell.set(entry.gridCellId, []);
      }
      valuesByCell.get(entry.gridCellId).push(entry.predictedValue);
    });
  ...
  const defaultFeatureLookup = new Map();
  featureEntries.forEach((entry) => {
    if (!entry.gridCellId || defaultFeatureLookup.has(entry.gridCellId)) {
      return;
    }
    defaultFeatureLookup.set(entry.gridCellId, entry.feature);
  });

  const backendInterpolatedSurface = getManualGridInterpolatedSurface();
  const backendInterpolatedSamples = Array.isArray(backendInterpolatedSurface?.samples)
    ? backendInterpolatedSurface.samples.filter((sample) => Number.isFinite(asFloat(sample?.predictedValue)))
    : [];
  const interpolationSubdivisions = getManualGridClientInterpolationSubdivisions(
    normalizedGridCells,
    backendInterpolatedSamples,
  );
  const directHeatSamples = backendOverlayBase?.mode === "numeric" && Array.isArray(backendOverlayBase.heat_samples)
    ? backendOverlayBase.heat_samples.filter((sample) => Number.isFinite(asFloat(sample?.predictedValue)))
    : buildManualGridDirectHeatSamples(normalizedGridCells, predictedLookup);
  const interpolatedHeatSamples = interpolationSubdivisions > 0
    ? buildManualGridHeatSamples(normalizedGridCells, predictedLookup, {
        subdivisionsPerCell: interpolationSubdivisions,
        weightPower: MANUAL_GRID_REPLICATED_WEIGHT_POWER,
        neighborCount: MANUAL_GRID_REPLICATED_NEIGHBOR_COUNT,
      })
    : [];
  ...
  manualGridOverlayComputationCache = {
    ...
    featureEntries,
    predictedLookup,
    predictedStats: backendOverlayBase?.mode === "numeric" && backendOverlayBase.predicted_stats
      ? backendOverlayBase.predicted_stats
      : buildManualGridPredictedStats(predictedLookup),
    defaultFeatureLookup,
    backendInterpolatedSamples,
    directHeatSamples,
    interpolatedHeatSamples,
    winningFeatureLookups: new Map(),
    profileHeatSampleLookups: new Map(),
  };
  if (backendOverlayBase?.mode === "categorical" && Array.isArray(backendOverlayBase.heat_samples)) {
    manualGridOverlayComputationCache.profileHeatSampleLookups.set(
      "all",
      backendOverlayBase.heat_samples.filter((sample) => sample && sample.gridCellId),
    );
  }
  return manualGridOverlayComputationCache;
};
...
const isFarmComparisonPrediction = () => {
  if (!isManualGridPredictionResult()) {
    return false;
  }
  const selectedIdHeader = String(datasetStore.prediction?.summary?.selected_id_header ?? getPredictionSelectedIdHeader() ?? "").trim().toLowerCase();
  return selectedIdHeader === "farm";
};

const isHeavyFarmManualGridPrediction = () => {
  if (!(isFarmComparisonPrediction() && isCategoricalPredictionColorMode())) {
    return false;
  }
  const featureCount = Number(datasetStore.prediction?.summary?.prediction_feature_count ?? 0);
  const gridCellCount = Number(datasetStore.prediction?.summary?.manual_bbox_grid?.grid_cell_count ?? 0);
  return featureCount <= 0 || gridCellCount <= 0 ? true : featureCount > gridCellCount;
};
...
const renderManualGridOverlay = (transform) => {
  ...
  if (!gridCells.length) {
    ...
    manualGridLastRenderedTransform = transform;
    return;
  }

  const overlayState = getManualGridOverlayComputationState(gridCells);
  const predictedLookup = overlayState.predictedLookup;
  const predictedStats = overlayState.predictedStats;
  const multiProfileMode = isCategoricalPredictionColorMode();
  const lightweightFarmComparisonMode = isFarmComparisonPrediction();
  ...
  const checkedLabels = getCheckedPredictionProfileLabels();
  const useCheckedLabels = predictionProfileSelectionTouched && checkedLabels.size > 0;
  let featureLookup = overlayState.defaultFeatureLookup;
  let profileHeatSamples = null;
  if (multiProfileMode) {
    const lookupKey = useCheckedLabels
      ? `checked:${Array.from(checkedLabels).map((label) => normalizePredictionCategoryLabel(label)).filter(Boolean).sort().join("|")}`
      : activeProfileLabel
        ? `active:${normalizePredictionCategoryLabel(activeProfileLabel)}`
        : "all";
    if (useCheckedLabels) {
      const serverOverlayKey = buildManualGridOverlaySelectionKey(checkedLabels);
    ...
    if (!overlayState.winningFeatureLookups.has(lookupKey)) {
      const allowedLabels = useCheckedLabels
        ? checkedLabels
        : activeProfileLabel
          ? new Set([activeProfileLabel])
          : null;
      overlayState.winningFeatureLookups.set(
        lookupKey,
        buildManualGridWinningFeatureLookupFromEntries(overlayState.featureEntries, allowedLabels),
      );
    }
    featureLookup = overlayState.winningFeatureLookups.get(lookupKey) ?? new Map();
    if (profileHeatSamples === null && !overlayState.profileHeatSampleLookups.has(lookupKey)) {
      overlayState.profileHeatSampleLookups.set(
        lookupKey,
        buildManualGridProfileHeatSamples(gridCells, featureLookup),
      );
    }
    if (profileHeatSamples === null) {
  ...
  const heatSamples = multiProfileMode
    ? profileHeatSamples ?? []
    : hasPredictionValues
      ? (useSmoothedNumericGrid
          ? (overlayState.backendInterpolatedSamples.length
              ? overlayState.backendInterpolatedSamples
              : overlayState.interpolatedHeatSamples)
          : overlayState.directHeatSamples)
      : [];
  const useGroupedCategoricalHeatPaths =
    multiProfileMode &&
    hasPredictionValues;
  const groupedHeatSamples = useGroupedCategoricalHeatPaths
    ? buildManualGridGroupedHeatSamples(
        heatSamples,
        transform,
        hasPredictionValues ? MANUAL_GRID_CATEGORICAL_SAMPLE_PADDING_PX : 0,
        (sample) => normalizePredictionCategoryLabel(sample?.profileLabel) || "__unclassified__",
      )
    : [];

  mapContext.manualGridHeatLayer
    .selectAll("path")
    .data(useGroupedCategoricalHeatPaths ? groupedHeatSamples : heatSamples, (sample) => sample.id)
    .join("path")
    .attr(
      "d",
      (sample) => (
        useGroupedCategoricalHeatPaths
          ? sample.path
          : buildManualGridScreenRectPath(
              sample,
              transform,
              hasPredictionValues
    ...
    .style("fill", (sample) => {
      if (multiProfileMode) {
        const profileLabel = useGroupedCategoricalHeatPaths ? sample.groupKey : sample.profileLabel;
        return toMarkerColorAlpha(
          getPredictionProfileColor(profileLabel),
          lightweightFarmComparisonMode ? 0.34 : 0.42,
        );
      }
      return toMarkerColorAlpha(
        getManualGridPredictedGradientColor(sample.predictedValue, predictedStats),
        useSmoothedNumericGrid ? 0.5 : 0.42,
      );
    })
    .style("opacity", (sample) => {
      ...
      if (!predictionProfileSelectionTouched) {
        return 1;
      }
      const highlightedLabel = getActivePredictionProfileLabel();
      if (!highlightedLabel) {
      ...
      const sampleLabel = String(
        useGroupedCategoricalHeatPaths ? sample.groupKey : sample.profileLabel ?? "",
      ).trim();
      return sampleLabel === highlightedLabel ? 1 : 0.12;
    })
    .style("stroke", "none")
  ...
  mapContext.manualGridHeatLayer
    .attr(
      ...
      hasPredictionValues
        ? (multiProfileMode
            ? `url(#${useBlurOnlyHeavyManualGrid ? MANUAL_GRID_LIGHT_SMOOTH_FILTER_ID : MANUAL_GRID_SMOOTH_FILTER_ID})`
            : useSmoothedNumericGrid
              ? `url(#${MANUAL_GRID_REPLICATED_SMOOTH_FILTER_ID})`
              : useBlurOnlyHeavyManualGrid
                ? `url(#${MANUAL_GRID_LIGHT_SMOOTH_FILTER_ID})`
                : null)
        : null,
    )
    .style(
      ...
      hasPredictionValues
        ? (multiProfileMode
            ? MANUAL_GRID_QUARTILE_HEAT_OPACITY
            : useSmoothedNumericGrid
              ? MANUAL_GRID_REPLICATED_HEAT_OPACITY
              : MANUAL_GRID_QUARTILE_HEAT_OPACITY)
        : 1,
    );
  ...
  mapContext.manualGridCellsLayer
    .selectAll("path")
    .data(renderedGridCells, (cell) => String(cell.grid_cell_id ?? ""))
    .join("path")
...
const setupMapBase = async () => {
  ...
  heatDefs
    .append("filter")
    .attr("id", MANUAL_GRID_LIGHT_SMOOTH_FILTER_ID)
    .attr("x", "-14%")
    .attr("y", "-14%")
    .attr("width", "128%")
    .attr("height", "128%")
  ...
  heatDefs
    .append("filter")
    .attr("id", MANUAL_GRID_REPLICATED_SMOOTH_FILTER_ID)
    .attr("x", "-30%")
    .attr("y", "-30%")
    .attr("width", "160%")
    .attr("height", "160%")
  ...
  const renderBase = (transform, { deferManualGridOverlay = false } = {}) => {
    ...
    if (deferManualGridOverlay && isManualGridPredictionResult() && getActiveManualGridPayload()) {
      if (!applyManualGridOverlayZoomTransform(transform)) {
  ...
  zoomBehavior = d3
    ...
    .on("zoom", (event) => {
      renderBase(event.transform, { deferManualGridOverlay: true });
    })
    .on("end", (event) => {
...
const clearMarkers = () => {
  ...
  resetManualGridLayerTransform();
  manualGridLastRenderedTransform = currentTransform;
  if (mapContext) {
...
const applyFeatureCollection = async (
  ...
  if (!(renderFlowMode === "saved_model" && activeDataViewTab === "prediction" && datasetStore.prediction?.summary?.climate_scope === "regional_manual")) {
    markerGroups.append("circle").attr("class", "marker-center").attr("r", 2.6).attr("cy", -2.5);
  }
  ...
  if (isMultiProfilePredictionResult()) {
    selectedFeature = null;
  } else if (isMultiGermplasmPredictionResult()) {
  ...
  if (isCategoricalPredictionColorMode()) {
    const dominantLabel = getDominantPredictionProfileLabel();
    setCheckedPredictionProfileLabels(dominantLabel ? [dominantLabel] : []);
    predictionProfileSelectionTouched = false;
    setActivePredictionProfileHighlight("");
  } else {
    setActivePredictionProfileHighlight("");
...
const handleFileUpload = async (event) => {
  ...
  try {
    ...
    await setLoadingState(
      94,
      "Rendering map in browser",
      "The pipeline is complete. Loading the final geospatial dataset and drawing the map locally in your browser.",
    );
...
const handleContinueAfterPreprocess = async () => {
  ...
  try {
    ...
    await sleepFrame();
    await triggerBrowserDownload(resolvePredictionOutputDownloadUrl(payload), {
      promptForFilename: false,
      suggestedFilename: `${String(payload?.source_name ?? "high_potential_sites").replace(/\.[^./]+$/, "") || "high_potential_sites"}.xlsx`,
      bypassDownloadGate: true,
    });
...
const handleExecuteSelectedGermplasm = async () => {
  ...
  try {
    ...
    const payload = await processFileWithStatusTimeline(file, {
      ...
      selectedGermplasmNames: getSelectedPredictionSourceNames(),
      selectedGermplasmRowIds: getSelectedPredictionRowIds(),
      selectedGermplasmProjectionMode,
      projectionTemplateWorkbook,
      forecastPlantingDate,
      forecastHarvestingDate,
...
if (germplasmSelectAllButton) {
  germplasmSelectAllButton.addEventListener("click", () => {
    selectedGermplasmNames = new Set(availableGermplasmNames);
    if (predictionBridgeState?.isAvailable) {
      predictionBridgeState.selectedNames = Array.from(selectedGermplasmNames);
    }
    renderGermplasmSelectionList();
...
if (germplasmSelectionIdSelect) {
  germplasmSelectionIdSelect.addEventListener("change", () => {
    ...
    if (!selectedHeader || selectedHeader === (selectedPredictionIdField || clearSelectionIdSentinel)) {
      return;
    }
...
if (predictionIdModalClose) {
  predictionIdModalClose.addEventListener("click", () => {
    cancelPredictionIdSelection().catch(console.error);
  });
}
...
if (predictionDownloadModalClose) {
  predictionDownloadModalClose.addEventListener("click", cancelPredictionDownloadDialog);
}
if (predictionDownloadModalBackdrop) {
  predictionDownloadModalBackdrop.addEventListener("click", cancelPredictionDownloadDialog);
}
if (predictionDownloadModalCancel) {
  predictionDownloadModalCancel.addEventListener("click", cancelPredictionDownloadDialog);
}
if (predictionDownloadModalSave) {
  predictionDownloadModalSave.addEventListener("click", confirmPredictionDownloadFromDialog);
}
if (predictionDownloadNameInput) {
  predictionDownloadNameInput.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      confirmPredictionDownloadFromDialog();
    }
  });
}
...
window.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && mapFrameExpanded) {
    setMapFrameExpanded(false).catch((error) => {
      console.error(error);
    });
    return;
  }
  ...
  if (event.key === "Escape" && predictionDownloadModal && !predictionDownloadModal.hidden) {
    cancelPredictionDownloadDialog();
  }
});
...
if (mapScreenshotButton) {
  mapScreenshotButton.addEventListener("click", () => {
    downloadMapScreenshot().catch((error) => {
      console.error(error);
      window.alert(error instanceof Error ? error.message : "The map screenshot could not be downloaded.");
    });
  });
}
...
if (mapToggleExpandButton) {
  mapToggleExpandButton.addEventListener("click", () => {
    toggleMapFrameExpanded().catch((error) => {
      console.error(error);
    });
  });
}
...
document.addEventListener("fullscreenchange", () => {
  mapFrameExpanded = getIsMapFrameFullscreen();
  syncMapViewportControls();
});
...
const init = async () => {
  ...
  await setupMapBase();
  await syncPersistedPipelineStateWithRuntime();
  if (!legacyPipelineDisabled) {
